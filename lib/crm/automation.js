...
import { sendChatwootOrderConfirmation, sendChatwootTrackingMessage } from './chatwoot.js';
import { sendWhatsAppOrderUpdate } from './wacli.js';
...
export function isOrderConfirmationEligible(order, options = {}) {
  if (!options.orderConfirmationEnabled || !isPaidOrder(order)) return false;
  if (['cancelled', 'canceled'].includes(normalizeStatus(order.status))) return false;
  const enabledAt = Date.parse(options.orderConfirmationEnabledAt || '');
  const orderCreatedAt = Date.parse(order.source_created_at || order.created_at || '');
  return Number.isFinite(enabledAt) && Number.isFinite(orderCreatedAt) && orderCreatedAt >= enabledAt;
}
...
export async function runAutomationCycle({ trigger = 'manual', force = false } = {}) {
  ...
  try {
    ...
    await runAbandonedCartSync(counters, errors);
    await runAmazonSync(counters, errors, force);
    await runWooSync(counters, errors, force);
    const settings = await loadSettings(supabase);
    const { getConfig } = await import('../../src/config.js');
    const config = applyCrmSettingsToConfig(getConfig(), settings);
    const confirmationOrders = await listOrderConfirmationCandidates(supabase, config.chatwoot);
    for (const order of confirmationOrders) {
      await sendOrderConfirmationOnce(supabase, order, config, counters);
    }
...
export async function syncOrderTrackingSideEffects(orderId, { reason = 'manual-awb' } = {}) {
  ...
  const counters = emptyCounters();
  await syncChannelAndMessage(supabase, order, mergedShipment, config, counters, reason);
...
export async function exportFedexBatchCsv(batchId) {
  ...
  const settings = await loadSettings(supabase);
  const { getConfig } = await import('../../src/config.js');
  const { buildFedexBatchUploadWorkbook, fedexBatchUploadFilename } = await import('../../src/internationalExport.js');
  const config = applyCrmSettingsToConfig(getConfig(), settings);
  const orders = (batch.fedex_export_items || []).map(item => ({
    ...(item.orders || {}),
    fedex_payload: item.csv_payload || {}
  }));
  const workbook = buildFedexBatchUploadWorkbook(orders, config);
  ...
  return { ok: true, filename: fedexBatchUploadFilename(batch.batch_number), workbook };
}
...
async function processOrder(supabase, order, config, counters) {
  ...
  if (lane.lane === 'post_awb') {
    ...
    await syncChannelAndMessage(supabase, order, shipment || orderToShipment(order), config, counters, 'post-awb');
...
async function bookAutomationShipment(supabase, order, lane, config, counters) {
  ...
  try {
    ...
    if (shipment.waybill) {
      await syncChannelAndMessage(supabase, order, shipment, config, counters, 'auto-booking');
    }
  } catch (error) {
    await recordAttempt(supabase, {
      orderId: order.id,
...
async function syncChannelAndMessage(supabase, order, shipment, config, counters, reason) {
  ...
  const actionType = isAmazon ? 'amazon-fulfillment' : isWoo ? 'woo-shipment-writeback' : 'wix-fulfillment';

  const attempt = await getAttemptState(supabase, order.id, actionType);
  if (attempt.attempts < RETRY_LIMIT || attempt.status === 'success') {
    try {
      await recordAttempt(supabase, { orderId: order.id, shipmentId: shipment.id, actionType, status: 'running', payload: { reason } });
      if (isAmazon) {
        const { syncShipmentTrackingToAmazon } = await import('../../src/amazonShipmentSync.js');
        await syncShipmentTrackingToAmazon(
          {
            ...order,
        ...
        counters.amazonUpdated = (counters.amazonUpdated || 0) + 1;
      } else if (isWoo) {
        const { syncShipmentTrackingToWoo } = await import('../../src/wooShipmentSync.js');
        ...
        if (result?.ok === false && !result?.skipped) {
          throw new Error(result.error || 'Woo shipment write-back failed');
        }
        counters.wooUpdated = (counters.wooUpdated || 0) + 1;
      } else {
      ...
      await recordAttempt(supabase, { orderId: order.id, shipmentId: shipment.id, actionType, status: 'success', payload: { reason, waybill } });
    } catch (error) {
      await recordAttempt(supabase, { orderId: order.id, shipmentId: shipment.id, actionType, status: 'failed', error: error.message, payload: { reason, waybill } });
      if ((attempt.attempts + 1) >= RETRY_LIMIT) await failOrder(supabase, order, actionType, error.message, { waybill });
    }
  }

  await sendTrackingMessageOnce(supabase, order, { ...shipment, waybill }, counters);
  await sendWhatsAppOrderUpdateOnce(supabase, order, { ...shipment, waybill }, counters);
}
...
async function sendOrderConfirmationOnce(supabase, order, config, counters) {
  const options = config.chatwoot || {};
  if (!isOrderConfirmationEligible(order, options)) return;
  if (order.last_message_type === 'order-confirmation') return;

  const messageType = 'order-confirmation';
  const existing = await safeMaybeSingle(
    supabase.from('customer_messages').select('*').eq('order_id', order.id).eq('message_type', messageType).eq('channel', 'chatwoot-whatsapp')
  );
  if (existing?.status === 'sent' || existing?.status === 'skipped') return;
  if (existing?.attempts >= RETRY_LIMIT) {
    counters.blocked += 1;
    return;
  }

  try {
    const result = await sendChatwootOrderConfirmation(order, {
      inboxId: options.inboxId,
      templateName: options.orderTemplateName,
      language: options.orderTemplateLanguage,
      category: options.orderTemplateCategory
    });
    const messageStatus = result.skipped ? 'skipped' : 'sent';
    await upsertCustomerMessage(supabase, order, {}, {
      messageType,
      channel: 'chatwoot-whatsapp',
      status: messageStatus,
      attempts: (existing?.attempts || 0) + 1,
      providerMessageId: result.providerMessageId || '',
      recipientRef: result.conversationId || '',
      content: result.content,
      error: result.reason || null
    });
    if (result.conversationId && messageStatus === 'sent') {
      await supabase.from('orders').update({
        chatwoot_conversation_id: result.conversationId,
        chatwoot_contact_id: result.contactId || order.chatwoot_contact_id || null,
        last_message_type: messageType,
        last_communication_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      }).eq('id', order.id);
    }
    counters.orderConfirmationsSent += messageStatus === 'sent' ? 1 : 0;
    counters.orderConfirmationsSkipped += messageStatus === 'skipped' ? 1 : 0;
  } catch (error) {
    ...
    counters.orderConfirmationsFailed += 1;
    if (attempts >= RETRY_LIMIT) await failOrder(supabase, order, 'chatwoot-order-confirmation', error.message);
  }
}

async function sendWhatsAppOrderUpdateOnce(supabase, order, shipment, counters) {
  const status = normalizeStatus(shipment.status || order.shipment_status || 'booked');
  const messageType = `order-update-${status}`;
  const existing = await safeMaybeSingle(
    supabase.from('customer_messages').select('*').eq('order_id', order.id).eq('message_type', messageType).eq('channel', 'whatsapp')
  );
  if (existing?.status === 'sent' || existing?.status === 'skipped') return;
  if (existing?.attempts >= RETRY_LIMIT) {
    counters.blocked += 1;
  ...
  try {
    const result = await sendWhatsAppOrderUpdate(order, shipment);
    const statusValue = result.skipped ? 'skipped' : 'sent';
    await upsertCustomerMessage(supabase, order, shipment, {
      messageType,
      channel: 'whatsapp',
      status: statusValue,
    ...
    counters.whatsAppSent += statusValue === 'sent' ? 1 : 0;
    counters.whatsAppSkipped += statusValue === 'skipped' ? 1 : 0;
  } catch (error) {
    await upsertCustomerMessage(supabase, order, shipment, {
      messageType,
      channel: 'whatsapp',
...
async function runAbandonedCartSync(counters, errors) {
  try {
    const { syncAbandonedCartLeads } = await import('./abandoned-carts.js');
    const result = await syncAbandonedCartLeads();
    counters.abandonedCartsSynced = result.synced || 0;
    counters.abandonedCartsRecovered = result.recovered || 0;
    if (!result.ok) errors.push(result.error || 'Abandoned-cart sync failed.');
  } catch (error) {
    errors.push(error.message);
  }
}
...
async function runWooSync(counters, errors, force) {
  try {
    const { runWooOrderSync } = await import('./woo-sync.js');
    const result = await runWooOrderSync({ reason: 'automation', force });
    counters.wooPulled = result.pulled || 0;
    counters.wooPersisted = result.persisted || 0;
    if (!result.ok) errors.push(result.error || 'WooCommerce sync failed.');
  } catch (error) {
    errors.push(error.message);
  }
}

async function runAmazonSync(counters, errors, force) {
  try {
    const { runAmazonOrderSync } = await import('./amazon-sync.js');
    const result = await runAmazonOrderSync({ reason: 'automation', force });
    counters.amazonPulled = result.pulled || 0;
    counters.amazonPersisted = result.persisted || 0;
    if (!result.ok) errors.push(result.error || 'Amazon sync failed.');
  } catch (error) {
    errors.push(error.message);
  }
}
...
async function runTrackingPoll(config, counters, errors) {
  try {
    ...
    const { createTrackingPickupFulfillHandler } = await import('../../src/shipmentChannelFulfillment.js');
    const fulfillOnPickup = createTrackingPickupFulfillHandler(config, {
      findOrderById: orderId => getAutomationOrder(supabase, orderId)
    });
    const sync = createDelhiveryTrackingSync(
      ...
      },
      {
        setTimer: () => null,
        clearTimer: () => null,
        onShipmentStatusChanged: async shipment => {
          const channelResult = await fulfillOnPickup(shipment);
          if (channelResult?.wix && !channelResult.wix.skipped) {
            counters.wixUpdated = (counters.wixUpdated || 0) + 1;
          }
          if (channelResult?.woo?.ok) {
            counters.wooUpdated = (counters.wooUpdated || 0) + 1;
          }
          const order = await getAutomationOrder(supabase, shipment.order_id);
          if (order) await sendWhatsAppOrderUpdateOnce(supabase, order, shipment, counters);
        }
      }
    );
...
async function listAutomationOrders(supabase, limit = 100) {
  const query = supabase
    ...
    .select(`
      ...
      source_created_at,created_at,updated_at,chatwoot_contact_id,
...
async function listOrderConfirmationCandidates(supabase, options = {}) {
  if (!options.orderConfirmationEnabled || !options.orderConfirmationEnabledAt) return [];
  const query = supabase
    .from('orders')
    .select(`
      id,wix_order_id,external_order_id,order_number,status,payment_status,source_created_at,created_at,
      raw_order,last_message_type,chatwoot_contact_id,chatwoot_conversation_id,
      customers(name,email,phone),
      order_items(product_name,quantity),
      shipping_address:customer_addresses!orders_shipping_address_id_fkey(name,phone,country)
    `)
    .gte('source_created_at', options.orderConfirmationEnabledAt)
    .in('payment_status', ['PAID', 'APPROVED', 'paid', 'approved'])
    .or('last_message_type.is.null,last_message_type.neq.order-confirmation')
...
async function createAutomationRun(supabase, trigger, startedAt) {
  const { data, error } = await supabase.from('automation_runs').insert({ trigger, started_at: startedAt }).select().single();
  ...
  await supabase.from('customer_messages').upsert(
    {
      ...
      message_type: fields.messageType || 'tracking-link',
      channel: fields.channel || 'chatwoot',
      status: fields.status,
      attempts: fields.attempts,
      recipient_ref: fields.recipientRef || order.chatwoot_conversation_id || '',
...
function emptyCounters() {
  return {
    ...
    whatsAppSent: 0,
    whatsAppSkipped: 0,
    orderConfirmationsSent: 0,
    orderConfirmationsSkipped: 0,
    orderConfirmationsFailed: 0,
    trackingPolled: 0,
    trackingUpdated: 0,
    trackingEvents: 0,
    blocked: 0,
