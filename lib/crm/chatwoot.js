...
export async function sendChatwootShipmentConfirmation(order, shipment, options = {}) {
  const config = chatwootConfig(options.env);
  const inboxId = String(options.inboxId || '');
  const phone = whatsappPhone(order);
  const customerName = String(order.customers?.name || order.shipping_address?.name || 'Customer').trim();
  const orderNumber = String(order.order_number || order.external_order_id || order.wix_order_id || order.id || '').trim();
  const waybill = String(shipment?.waybill || order.shipment_waybill || '').trim();
  const content = `Hi ${customerName}, your order ${orderNumber} has left our facility and is on its way to you!\n\nYour tracking ID is ${waybill}.\n\nClick below to track your package.`;
  if (!config.baseUrl || !config.accountId || !config.token) return { skipped: true, reason: 'missing-chatwoot-config', content };
  ...
  if (!waybill) return { skipped: true, reason: 'missing-tracking-id', content };

  const fetchImpl = options.fetchImpl || fetch;
  const contact = await findOrCreateContact(config, inboxId, phone, customerName, fetchImpl);
  const conversation = await findOrCreateConversation(config, inboxId, contact, fetchImpl);
  const buttonUrl = String(options.trackingButtonUrl || 'https://holdmythrottle.com/track/{{1}}');
  const payload = await chatwootRequest(config, `/api/v1/accounts/{accountId}/conversations/${encodeURIComponent(conversation.id)}/messages`, {
    method: 'POST',
    body: {
      content,
      message_type: 'outgoing',
      ...
      content_type: 'text',
      template_params: {
        name: String(options.templateName || 'shipment_confirmation_3'),
        category: String(options.category || 'UTILITY').toUpperCase(),
        language: String(options.language || 'en_US'),
        processed_params: {
          body: { 1: customerName, 2: orderNumber, 3: waybill },
          buttons: [{ type: 'url', parameter: waybill, url: buttonUrl, variables: ['1'] }]
        }
      }
    }
  }, fetchImpl);
