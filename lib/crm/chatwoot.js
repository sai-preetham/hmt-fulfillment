...
export async function sendChatwootAbandonedCartFollowUp(lead, options = {}) {
  const config = chatwootConfig(options.env);
  const inboxId = String(options.inboxId || '');
  const templateName = String(options.templateName || 'abandoned_cart');
  const language = String(options.language || 'en');
  const category = String(options.category || 'MARKETING').toUpperCase();
  const phone = whatsappPhone({ whatsapp_number: lead.phone });
  const customerName = String(lead.customer_name || 'Customer').trim();
  const product = abandonedCartProduct(lead);
  const buttonUrl = String(options.buttonUrl || 'https://www.holdmythrottle.com/product-page/{{1}}');
  const checkoutSuffix = dynamicUrlParameter(lead.checkout_url, buttonUrl);
  const content = [
    `Hello ${customerName}`,
    'Gautama here from Hold My Throttle.',
    `We see that you tried to purchase Cruise Control for ${product}, but could not complete it.`,
  ...
  if (!inboxId) return { skipped: true, reason: 'missing-whatsapp-inbox', content };
  if (!phone) return { skipped: true, reason: 'missing-whatsapp-number', content };
  if (!checkoutSuffix) return { skipped: true, reason: 'checkout-url-incompatible-with-template', content };

  const fetchImpl = options.fetchImpl || fetch;
  const contact = await findOrCreateContact(config, inboxId, phone, customerName, fetchImpl);
  const conversation = await findOrCreateConversation(config, inboxId, contact, fetchImpl);
  const payload = await chatwootRequest(
    config,
    `/api/v1/accounts/{accountId}/conversations/${encodeURIComponent(conversation.id)}/messages`,
    {
      ...
      body: {
        ...
        template_params: {
          ...
          language,
          processed_params: {
            body: { a: customerName, b: product },
            buttons: [{
              type: 'url',
              parameter: checkoutSuffix,
              url: buttonUrl,
              variables: ['1']
            }]
          }
...
function abandonedCartProduct(lead) {
  const item = (lead.items || [])[0] || {};
  const modelMeta = (item.meta_data || []).find(meta => /bike|model|vehicle/i.test(String(meta?.key || meta?.display_key || '')));
  const raw = modelMeta?.value || modelMeta?.display_value || item.name || item.productName?.translated || item.productName?.original || item.productName || 'your motorcycle';
  return String(raw)
    .replace(/^HMT\s+Cruise\s+(?:Control|Kit)\s*(?:for|-)?\s*/i, '')
    .replace(/^Cruise\s+Control\s+(?:for\s+)?/i, '')
    .trim() || 'your motorcycle';
}

function dynamicUrlParameter(value, templateUrl) {
  if (!value) return '';
  try {
    const url = new URL(value);
    const template = new URL(templateUrl.replace('{{1}}', ''));
    if (url.origin !== template.origin) return '';
    const prefix = template.pathname.replace(/^\//, '');
    const path = url.pathname.replace(/^\//, '');
    if (prefix && !path.startsWith(prefix)) return '';
    return `${path.slice(prefix.length)}${url.search}${url.hash}`;
  } catch {
