...
export async function sendChatwootOrderConfirmation(order, options = {}) {
  ...
  const updatedContact = await updateContactForOrder(config, contact, order, template, fetchImpl);
  const conversation = await findOrCreateConversation(config, inboxId, updatedContact, fetchImpl);
...
async function updateContactForOrder(config, contact, order, template, fetchImpl) {
  const existingAttributes = contact.custom_attributes || {};
  const orderId = template.orderNumber;
  const alreadyCounted = String(existingAttributes.last_order_id || '') === orderId;
  const currentOrderCount = Number(existingAttributes.order_count);
  const orderCount = alreadyCounted
    ? (Number.isFinite(currentOrderCount) ? currentOrderCount : 1)
    : (Number.isFinite(currentOrderCount) ? currentOrderCount : 0) + 1;
  const wixContactId = order.customers?.wix_contact_id || order.raw_order?.buyerInfo?.contactId || '';
  const customAttributes = {
    ...existingAttributes,
    bike_model: template.product,
    is_paid_customer: true,
    order_count: orderCount,
    last_order_id: orderId
  };
  if (wixContactId) customAttributes.wix_contact_id = String(wixContactId);

  const updated = await chatwootRequest(
    config,
    `/api/v1/accounts/{accountId}/contacts/${encodeURIComponent(contact.id)}`,
    { method: 'PUT', body: { custom_attributes: customAttributes } },
    fetchImpl
  );
  return updated.payload?.contact || updated.payload || updated.contact || { ...contact, custom_attributes: customAttributes };
}
