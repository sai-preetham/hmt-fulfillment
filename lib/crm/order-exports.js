...
import { postDiscordChatwootDailyTracker } from './chatwoot.js';
...
export async function runOrderExports({ trigger = 'manual' } = {}) {
  ...
  try {
    ...
    const [sheet, discord, chatwootDiscord] = await Promise.all([
      syncGoogleSheet(orders),
      postDiscordDailyOrders(new Date(), bounds),
      postDiscordChatwootDailyTracker(bounds)
    ]);
    return {
...
export async function getPreviousDayOrderReport(now = new Date(), suppliedBounds) {
  const supabase = createServiceClient();
  if (!supabase) throw new Error('Supabase service client is not configured.');
  const bounds = suppliedBounds || previousCalendarDayBounds(now);
...
export async function postDiscordDailyOrders(now = new Date(), bounds) {
  ...
  const report = await getPreviousDayOrderReport(now, bounds);
