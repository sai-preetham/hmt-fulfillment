...
export async function markShipmentPickedUp(orderId, shipmentId) {
  ...
  try {
    const { getConfig } = await import('@/src/config.js');
    const { writeWooShipmentOnPickedUp, isWooCommerceOrder } = await import('@/src/wooShipmentSync.js');
    if (isWooCommerceOrder(order)) {
      wooWriteback = await writeWooShipmentOnPickedUp(order, updated, getConfig());
    }
  } catch (error) {
    console.error(`[woo-shipment-writeback] picked_up soft-fail for order ${orderId}: ${error.message}`);
