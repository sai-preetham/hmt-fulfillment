...
export async function getDelhiveryPickupWorkspace() {
  const supabase = createServiceClient();
  if (!supabase) return { needsPickup: [], pickedUp: [], exceptions: [], updatedAt: null };
  const activeStatuses = ['pending', 'booked', 'shipment_booked', 'pickup_pending', 'pickup-pending'];
  const completedStatuses = ['picked_up', 'picked-up', 'dispatched', 'in_transit', 'in-transit', 'out_for_delivery', 'out-for-delivery', 'delivered', 'rto'];
  const exceptionStatuses = ['failed', 'failed_delivery', 'pickup_error', 'cancelled', 'returned'];
  const { data, error } = await supabase
    .from('shipments')
    .select('id, order_id, order_number, waybill, pickup_location, status, courier_code, weight_grams, updated_at, error, orders(order_number, external_order_id, customers(name, phone))')
    .eq('courier_code', 'delhivery')
    .in('status', [...activeStatuses, ...completedStatuses, ...exceptionStatuses])
    .not('waybill', 'is', null)
    .order('updated_at', { ascending: false });
  if (error) return { needsPickup: [], pickedUp: [], exceptions: [], updatedAt: null };
  const normalized = (data || []).map(shipment => ({
    ...shipment,
    order_number: shipment.order_number || shipment.orders?.order_number || shipment.orders?.external_order_id || '',
    customer_name: shipment.orders?.customers?.name || '',
    customer_phone: shipment.orders?.customers?.phone || '',
    status_key: String(shipment.status || '').toLowerCase().replaceAll('_', '-')
  }));
  const needsPickup = groupPickupShipments(normalized.filter(shipment => activeStatuses.includes(shipment.status) || activeStatuses.includes(shipment.status_key)));
  return {
    needsPickup,
    pickedUp: normalized.filter(shipment => completedStatuses.includes(shipment.status) || completedStatuses.includes(shipment.status_key)),
    exceptions: normalized.filter(shipment => exceptionStatuses.includes(shipment.status) || exceptionStatuses.includes(shipment.status_key)),
...
function groupPickupShipments(shipments) {
  ...
  for (const shipment of shipments) {
    const location = String(shipment.pickup_location || '').trim() || 'Unassigned pickup location';
    const group = groups.get(location) || { location, shipments: [], requestableCount: 0, requestedCount: 0 };
    group.shipments.push(shipment);
    if (['pending', 'booked', 'shipment-booked'].includes(shipment.status_key)) group.requestableCount += 1;
    if (['pickup-pending'].includes(shipment.status_key)) group.requestedCount += 1;
    groups.set(location, group);
  }
  return [...groups.values()].sort((left, right) => left.location.localeCompare(right.location));
}
