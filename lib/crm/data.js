import { isAwaitingWarehousePickup, canConfirmPickup, hasCompletedPickup } from './pickup.js';
export { isAwaitingWarehousePickup } from './pickup.js';
import { createServiceClient } from '@/lib/supabase/server';
import { findMatchingShipment, isDuplicateShipmentError } from './shipment-dedup';
...
} from './direct-courier-booking.js';
export {
  directCourierBookingDeniedReason,
...
export async function getPickupWorkspace() {
  ...
  const awaiting = normalized.filter(shipment => isAwaitingWarehousePickup(shipment) && !hasCompletedPickup(shipment));
  ...
  return {
    ...
    pickedUp: normalized.filter(hasCompletedPickup),
...
export async function markShipmentPickedUp(orderId, shipmentId) {
  ...
  const { fulfillShipmentChannelsOnPickup } = await import('@/src/shipmentChannelFulfillment.js');
  const config = applyCrmSettingsToConfig(getConfig(), await getCrmSettings());

  if (order.wix_order_id && !config.wix.fulfillmentSyncEnabled) {
    return {
      ok: false,
      shipment: updated,
      error: 'Pickup recorded. Enable Wix fulfillment in settings, then retry Wix fulfillment below.'
    };
  }

  const channelResult = await fulfillShipmentChannelsOnPickup(order, updated, config, { softFailWix: false });
  const wooWriteback = channelResult.woo;

  let wixSync = null;
  if (order.wix_order_id) {
    wixSync = await findOrderById(orderId);
    const skippedAlready = channelResult.wix?.skipped && channelResult.wix?.reason === 'already-fulfilled';
    if (!skippedAlready && wixSync?.wix_fulfillment_status !== 'fulfilled') {
      return {
        ok: false,
        shipment: updated,
        wix_sync: wixSync,
        woo_writeback: wooWriteback,
        error: `Pickup recorded, but Wix fulfillment failed: ${wixSync?.wix_fulfillment_error || channelResult.wix?.error || 'Please retry Wix fulfillment below.'}`
      };
    }
  }
  ...
  let whatsapp = null;
  try {
    const { sendPickupConfirmationOnce } = await import('./whatsapp-notifications.js');
    whatsapp = await sendPickupConfirmationOnce(order, updated);
  } catch (error) {
    whatsapp = { status: 'failed', error: error.message };
  }
  const message = order.wix_order_id
    ? 'Picked up. Wix fulfilled and tracking sent.'
    : isWooishMessage(order, wooWriteback)
      ? 'Shipment marked picked up. WooCommerce tracking meta updated.'
      : 'Shipment marked picked up.';
...
function isWooishMessage(order, wooWriteback) {
  return Boolean(order?.source === 'woocommerce' || order?.woo_order_id) && wooWriteback?.ok === true;
}
...
export async function bookShipment(orderId, payload) {
  ...
  const existingShipment = await findShipmentWithSameAwb(supabase, orderId, payload.courier, payload.awb_number);
  if (existingShipment) {
    return finishManualShipmentSave(supabase, orderId, payload, trackingUrl, existingShipment, true);
  }
  ...
  let { data: shipment, error: shipmentError } = await insertShipmentWithLegacyFallback(supabase, shipmentInsert);
  if (shipmentError && isDuplicateShipmentError(shipmentError)) {
    shipment = await findShipmentWithSameAwb(supabase, orderId, payload.courier, payload.awb_number);
    shipmentError = shipment ? null : shipmentError;
  }
  if (shipmentError) return { ok: false, error: shipmentError.message };
  return finishManualShipmentSave(supabase, orderId, payload, trackingUrl, shipment, false);
}

async function findShipmentWithSameAwb(supabase, orderId, courierCode, waybill) {
  const { data, error } = await supabase
    .from('shipments')
    .select('*')
    .eq('order_id', orderId)
  ...
  return findMatchingShipment(data, { orderId, waybill });
}

async function finishManualShipmentSave(supabase, orderId, payload, trackingUrl, shipment, deduplicated) {
  ...
  try {
    const { findOrderById } = await import('@/src/store.js');
    const { getConfig } = await import('@/src/config.js');
    const { writeWooShipmentOnBooked, isWooCommerceOrder } = await import('@/src/wooShipmentSync.js');
    const order = await findOrderById(orderId);
    if (order && isWooCommerceOrder(order)) {
      wooWriteback = await writeWooShipmentOnBooked(
        order,
        {
          ...shipment,
          waybill: payload.awb_number || shipment.waybill,
          courier_code: payload.courier || shipment.courier_code,
          tracking_url: trackingUrl,
          status: 'booked'
        },
        getConfig()
    ...
    console.error(`[woo-shipment-writeback] booked soft-fail for order ${orderId}: ${error.message}`);
  ...
  return {
    ...
    woo_writeback: wooWriteback,
    deduplicated,
    message: deduplicated ? 'This AWB was already saved for the order; the existing shipment was reused.' : 'Shipment saved.'
  };
}
...
async function bookCourierShipment(orderId, payload, supabase) {
  ...
  const denied = directCourierBookingDeniedReason(order);
  if (denied) {
  ...
  const { getConfig } = await import('@/src/config.js');
  const { bookWixOrder, bookWixOrderById, bookAmazonOrder, bookAmazonOrderById, bookWooCommerceOrder, bookWooCommerceOrderById } = await import('@/src/booking.js');
  ...
  try {
    const source = normalizeDirectBookingSource(order.source);
    let result;
    if (source === 'amazon') {
      result = order.raw_order
        ? await bookAmazonOrder(order.raw_order, config, bookingMetadata)
        : await bookAmazonOrderById(order.id, config, bookingMetadata);
    } else if (source === 'woocommerce' || source === 'woo') {
      result = order.raw_order
        ? await bookWooCommerceOrder(order.raw_order, config, { ...bookingMetadata, dbOrderId: order.id })
        : await bookWooCommerceOrderById(order.id, config, bookingMetadata);
    } else {
      result = order.raw_order
...
async function getShipmentBookingOrder(supabase, orderId) {
  const primary = await supabase
    .from('orders')
    .select('id,wix_order_id,woo_order_id,external_order_id,order_number,source,payment_status,raw_order,shipping_address:customer_addresses!orders_shipping_address_id_fkey(name,phone,address_line1,address_line2,city,state,postal_code,country)')
    .eq('id', orderId)
    .maybeSingle();
