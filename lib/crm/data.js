...
const PICKUP_TERMINAL_STATUSES = [
  'cancelled', 'canceled', 'returned', 'rto'
];
...
export function isAwaitingWarehousePickup(shipment = {}) {
  ...
  const key = String(shipment.status_key || shipment.status || '').trim().toLowerCase().replaceAll('_', '-');
  if (!key) return true;
  const blocked = new Set([
    ...PICKUP_COLLECTED_STATUSES,
 ...
 * Awaiting = has waybill AND status is not picked-up / in-transit / OFD /
...
export async function getPickupWorkspace() {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      weight_grams, updated_at, created_at, error, tracking_url,
      orders(
        order_number, external_order_id, tracking_url, shipment_booked_at,
    ...
    .order('updated_at', { ascending: false })
    .limit(2500);
  if (error) return { needsPickup: [], pickedUp: [], exceptions: [], updatedAt: null, awaitingCount: 0 };

  const now = Date.now();
  const normalized = (data || [])
    .filter(shipment => String(shipment.waybill || '').trim())
    .map(shipment => {
      ...
      const trackingUrl = shipment.tracking_url || shipment.orders?.tracking_url || trackingUrlFor(carrier, shipment.waybill);
      return {
        ...shipment,
...
export async function markShipmentPickedUp(orderId, shipmentId) {
  ...
  if (!String(shipment.waybill || '').trim()) return { ok: false, error: 'Shipment has no AWB.' };
  ...
  try {
    ...
    const { getCrmSettings } = await import('@/lib/crm/data-settings');
    const { applyCrmSettingsToConfig } = await import('@/lib/crm/settings');
    const { markShipmentPickedUpInWix } = await import('@/src/wixShipmentSync.js');
    wixSync = await markShipmentPickedUpInWix(
      { ...updated, status: 'picked-up' },
      applyCrmSettingsToConfig(getConfig(), await getCrmSettings())
    );
  } catch (wixError) {
    wixSync = { status: 'failed', error: wixError.message };
  }
  ...
  return {
    ...
    shipment: updated,
    wix_sync: wixSync,
    message: 'Marked picked up and synced Wix fulfillment when needed.'
  };
}
...
export async function getDashboardSummary() {
  ...
  return {
    ...
    pickupPending: (await getPickupWorkspace()).awaitingCount,
...
export async function bookShipment(orderId, payload) {
  ...
  // Manual AWBs are used for international carrier labels. They have already
  // been created outside the courier API, so notify Wix immediately instead of
  // waiting for the domestic tracking poller to observe a pickup.
  let wixSync = null;
  try {
    ...
    if (order?.wix_order_id) {
      wixSync = await fulfillManualShipmentInWix(
        order,
        {
          ...shipment,
          waybill: payload.awb_number,
          courier_code: payload.courier || shipment.courier_code,
          courier_service_code: payload.service_code || shipment.courier_service_code,
          service_mode: payload.service_code || shipment.service_mode || payload.courier,
          tracking_url: trackingUrl
        },
        applyCrmSettingsToConfig(getConfig(), await getCrmSettings())
      );
    ...
    // The local AWB remains safely saved. Wix failure details are persisted by
    // the sync helper so operators can retry from the order page.
    wixSync = { status: 'failed', error: wixError.message };
  }
...
async function bookCourierShipment(orderId, payload, supabase) {
  ...
  try {
    ...
    const { error: updateError } = await updateOrderShipmentSummary(supabase, orderId, {
      ...
      shipment_status: awb ? 'pickup_pending' : normalizeBookedShipmentStatus(shipment.status),
