...
function renderInternationalLabelPdf(model) {
  ...
  const shipperPhone = process.env.INVOICE_SHIPPER_PHONE || process.env.INVOICE_SELLER_PHONE || '8904137604';
  ...
  const ioss = process.env.INVOICE_IOSS_NUMBER || 'N/A';
  const terms = process.env.INVOICE_INCO_TERMS || 'CIF';
  const bankAdCode = process.env.INVOICE_BANK_AD_CODE || '00000006360108';
  const placeOfSupply = process.env.INVOICE_PLACE_OF_SUPPLY || 'Bangalore';
  const stateCode = process.env.INVOICE_STATE_CODE || 'KA';
  const stateName = process.env.INVOICE_STATE_NAME || 'Karnataka';
  const supplyDate = formatIsoDate(order.order_date || new Date().toISOString());
  const pieces = order.quantity || model.items.reduce((sum, item) => sum + item.quantity, 0) || 1;
  ...
  strokeRect(commands, left, left, width, pageHeight - left * 2, '0 0 0 RG');

  let y = pageHeight - left;
  writeAlignedCell(text, left, y - 13.2, width, 'TAX INVOICE', {
    size: 10.3,
    bold: true,
    align: 'center',
    color: '0.00 0.20 0.45 rg'
  });
  writeAlignedCell(text, left, y - 25, width, 'COMMERCIAL EXPORT', {
    size: 5.9,
    bold: true,
    align: 'center',
    color: '0.00 0.20 0.45 rg'
  ...
  y -= 27.2;
  line(commands, left, y, right, y, '0 0 0 RG');

  const exporterTop = y;
  text(left + 3, y - 9.2, 'EXPORTER', { size: 5.9, bold: true, color: '0.00 0.20 0.45 rg' });
  text(left + 43, y - 9.2, shipperName, { size: 8.3, bold: true });
  const exporterAddressLines = wrapText(shipperAddress, 96).slice(0, 2);
  exporterAddressLines.forEach((value, index) => text(left + 3, y - 20 - index * 8, value, { size: 5.55 }));
  text(left + 3, y - 38.2, `${shipperPhone}  |  ${shipperEmail}`, { size: 5.55 });
  y = exporterTop - 43.1;
  line(commands, left, y, right, y, '0.55 0.55 0.55 RG');

  const legalRows = [
    [`GSTIN: ${seller.gstin || '-'}`, `IEC/PAN: ${iec}`, `Bank AD Code: ${bankAdCode}`],
    [`Invoice: ${invoice.number || '-'}`, `Date: ${supplyDate}`, `AWB: ${invoice.awb || '-'}`],
    [`LUT: ${lut}`, `IOSS: ${ioss}`, 'Reverse charge: No'],
    [`Export using e-commerce: ${isExportUsingEcommerce(order) ? 'Yes' : 'No'}`, `Incoterm: ${terms}`, 'Origin: India'],
    [`Destination: ${destination}`, `Currency: ${currency}`, `Pieces: ${pieces}`],
    [`Place of supply: ${placeOfSupply} (${stateCode})`, '', '']
  ];
  const metaRowHeight = 12;
  legalRows.forEach((row, rowIndex) => {
    const cells = row.filter(Boolean);
    const cellWidth = (width - 6) / Math.max(cells.length, 1);
    cells.forEach((value, index) => writeAlignedCell(text, left + 3 + index * cellWidth, y - 8.3, cellWidth - 3, value, {
      size: rowIndex === 2 && index === 0 ? 5.25 : 5.35,
      bold: rowIndex === 1,
      align: 'left'
    }));
    y -= metaRowHeight;
    line(commands, left, y, right, y, '0.65 0.65 0.65 RG');
  });

  rect(commands, left, y - 13.6, width, 13.6, '0.90 0.93 0.96 rg');
  line(commands, left, y, right, y, '0.65 0.65 0.65 RG');
  text(left + 3, y - 9.5, 'CONSIGNEE / SHIP TO', { size: 6.3, bold: true });
  y -= 13.6;
  line(commands, left, y, right, y, '0.65 0.65 0.65 RG');

  const consigneeTop = y;
  const consigneeLines = [
    shippingAddress.name || order.customer_name || 'Not provided',
    ...
    destination,
    [`Tel: ${shippingAddress.phone || order.phone || '-'}`, order.email ? `Email: ${order.email}` : ''].filter(Boolean).join(' | ')
  ].filter(Boolean);
  let consigneeCursor = y - 9;
  let consigneeLineCount = 0;
  for (const value of consigneeLines) {
    const lines = wrapText(value, 92).slice(0, 2);
    for (const part of lines) {
      if (consigneeLineCount >= 6) break;
      text(left + 3, consigneeCursor, part, { size: consigneeLineCount === 0 ? 5.9 : 5.55, bold: consigneeLineCount === 0 });
      consigneeCursor -= 8;
      consigneeLineCount += 1;
    }
    if (consigneeLineCount >= 6) break;
  }
  y = consigneeTop - 45.9;
  ...
  const columns = [
    { label: '#', x: left, w: 12.5, align: 'center' },
    { label: 'Description', x: left + 12.5, w: 94.6, align: 'left' },
    { label: 'HSN', x: left + 107.1, w: 26.7, align: 'center' },
    { label: 'Qty', x: left + 133.8, w: 23.5, align: 'center' },
    { label: 'Unit price', x: left + 157.3, w: 47.4, align: 'right' },
    { label: 'IGST', x: left + 204.7, w: 24.9, align: 'center' },
    { label: 'Amount', x: left + 229.6, w: width - 229.6, align: 'right' }
  ];
  const headerHeight = 16;
  const headerTop = y;
  const headerTextY = headerTop - 10.4;
  rect(commands, left, headerTop - headerHeight, width, headerHeight, '0.90 0.93 0.96 rg');
  columns.forEach(column => writeAlignedCell(text, column.x + 2, headerTextY, column.w - 4, column.label, {
    size: 5.0,
    bold: true,
    align: column.align,
    color: '0.00 0.20 0.45 rg'
  }));
  line(commands, left, headerTop, right, headerTop, '0 0 0 RG');
  line(commands, left, headerTop - headerHeight, right, headerTop - headerHeight, '0.65 0.65 0.65 RG');
  y -= headerHeight;
  labelItems.slice(0, 6).forEach((item, itemIndex) => {
    const lines = wrapText(item.name, 29).slice(0, 2);
    const rowHeight = Math.max(12, lines.length * 5.8 + 4);
    const rowTextY = y - 7.7;
    writeAlignedCell(text, columns[0].x + 2, rowTextY, columns[0].w - 4, String(itemIndex + 1), { size: 5.0, align: 'center' });
    lines.forEach((value, index) => text(columns[1].x + 2, rowTextY - index * 5.8, value, { size: 5.0 }));
    writeAlignedCell(text, columns[2].x + 2, rowTextY, columns[2].w - 4, item.hsn || '-', { size: 5.0, align: 'center' });
    writeAlignedCell(text, columns[3].x + 2, rowTextY, columns[3].w - 4, String(item.quantity), { size: 5.0, align: 'center' });
    writeAlignedCell(text, columns[4].x + 2, rowTextY, columns[4].w - 4, numberText(item.unitPrice), { size: 5.0, align: 'right' });
    writeAlignedCell(text, columns[5].x + 2, rowTextY, columns[5].w - 4, '0%', { size: 5.0, align: 'center' });
    writeAlignedCell(text, columns[6].x + 2, rowTextY, columns[6].w - 4, numberText(item.lineTotal), { size: 5.0, align: 'right' });
    y -= rowHeight;
    line(commands, left, y, right, y, '0.55 0.55 0.55 RG');
  });
  const itemTableBottom = y;
  columns.forEach(column => line(commands, column.x, headerTop, column.x, itemTableBottom, '0.65 0.65 0.65 RG'));
  line(commands, right, headerTop, right, itemTableBottom, '0.65 0.65 0.65 RG');

  text(left + 3, y - 7.8, `Total before tax: ${currency} ${numberText(merchandiseTotal)}`, { size: 5.35, bold: true });
  y -= 10.8;
  line(commands, left, y, right, y, '0 0 0 RG');

  const declarationTop = y;
  const declarationHeight = 72.3;
  const splitX = left + width * 0.545;
  line(commands, splitX, declarationTop, splitX, declarationTop - declarationHeight, '0.35 0.35 0.35 RG');
  text(left + 3, y - 8, 'EXPORT DECLARATION', { size: 5.8, bold: true, color: '0.00 0.20 0.45 rg' });
  writeWrappedText(text, left + 3, y - 18.2, 'Supply meant for export under Letter of Undertaking without payment of IGST.', 49, {
    size: 4.9,
    lineHeight: 5.8,
    maxLines: 3,
    bold: true
  });
  writeWrappedText(text, left + 3, y - 37.6, 'We declare that this invoice shows the actual price of the goods described and all particulars are true and correct.', 49, {
    size: 4.8,
    lineHeight: 5.7,
    maxLines: 4
  });
  ...
  const totalRows = [
    ['Product Amount', merchandiseTotal],
    ['Shipping', totals.shippingGross],
    ['Insurance', 0],
    ['Total IGST', 0],
    ['TOTAL AFTER TAX', totals.grandTotal]
  ];
  let totalsY = declarationTop - 31;
  totalRows.forEach(([label, value], index) => {
    const bold = index === totalRows.length - 1;
    if (bold) rect(commands, splitX + 1, totalsY - 3.4, right - splitX - 2, 8.5, '0.94 0.96 0.99 rg');
    text(splitX + 7, totalsY, label, { size: 5.1, bold, color: bold ? '0.00 0.20 0.45 rg' : '0 0 0 rg' });
    const amountText = `${currency} ${numberText(value)}`;
    writeAlignedCell(text, splitX + 78, totalsY, right - splitX - 84, amountText, { size: 5.1, bold, align: 'right', color: bold ? '0.00 0.20 0.45 rg' : '0 0 0 rg' });
    totalsY -= 8;
  });
  y = declarationTop - declarationHeight;
  line(commands, left, y, right, y, '0 0 0 RG');

  writeAlignedCell(text, splitX + 7, y - 8.4, right - splitX - 10, 'ORIGINAL FOR RECIPIENT / CUSTOMS', { size: 5.8, bold: true, align: 'right' });
  text(left + 3, y - 24.5, 'Electronically generated invoice under the IT Act, 2000. Signature not required.', { size: 4.9 });
...
function consolidatedInternationalLabelItems(model, merchandiseTotal) {
  const primary = model.items.find(item => /hold my throttle/i.test(item.name)) || model.items[0] || {};
  const quantity = Number(model.order.quantity || primary.quantity || 1) || 1;
  return [{
    ...primary,
