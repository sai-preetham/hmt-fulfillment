...
export function isPublicRoute(request) {
  ...
  if (pathname === '/bulk' || pathname === '/bulk/') return true;
  const forwardedHost = request.headers?.get?.('x-forwarded-host') || request.headers?.get?.('host') || request.nextUrl?.hostname || '';
  const hostname = String(forwardedHost).split(',')[0].trim().split(':')[0].toLowerCase();
  return hostname === 'track.holdmythrottle.com' && /^\/(?=[A-Za-z0-9-]*\d)[A-Za-z0-9-]{5,64}\/?$/.test(pathname);
}
