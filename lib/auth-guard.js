...
export function isAutomationAuthBypassAllowed(request, env = process.env) {
  ...
  if (!pathname.startsWith('/api/automation') && pathname !== '/api/tracking/sync' && pathname !== '/api/integrations/orders/export' && pathname !== '/api/integrations/chatwoot/daily-report') return false;
